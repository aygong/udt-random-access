function [real_myo_sim] = unknown_myo_sim()
...
user_index = 1:N;
real_myo_sim = zeros(simu_frames, length(num_frames));
...
for nf = 1:length(num_frames)
    ...
    parfor SI = 1:simu_repeat(nf)
        ...
        if strcmp(metric_type, 'UDT')
            real_myo_sim(SI, nf) = SIMU / num_frames(nf) / D;
        else
            real_myo_sim(SI, nf) = 1 - SIMU / num_frames(nf) / dot(initial_belief, state);
        end
    end
    
    % Print the metric
    fprintf("- log10(num_frames) = %d, real_myo_sim = %.6f\n", ...
        log10(num_frames(nf)), sum(real_myo_sim(:, nf) / simu_repeat(nf)));
end

real_myo_sim = sum(real_myo_sim, 1) ./ simu_repeat;